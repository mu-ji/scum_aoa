...
def music_algorithm(signal_array, num_sources, angle_range, frequency):
    ...
    for i, angle in enumerate(angle_range):
        steering_vector = np.exp(1j * (2 * np.pi * frequency / SPEED_OF_LIGHT) * 0.0375 * np.arange(signal_array.shape[0]) * np.sin(np.radians(angle)))
...
def cal_angle_with_music(I_data, Q_data, angle_change_1us):
    ...
    music_spectrum, music_angle = music_algorithm(x_n[0:3], 1, np.linspace(-90, 90, 180), frequency)
